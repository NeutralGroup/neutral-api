...
def parse_args():
    ...
    parser.add_argument(
        ...
        help='Private key src address'
    )
    parser.add_argument(
        '--dst-address',
        dest='dst_address',
        type=str,
        default='0x1a63c94aae69d28ddd810e4e9e3113eeecfcfe2d',   # Ganache test address
        help='Ethereum address for the destination of the transaction'
    )
    parser.add_argument(
...
class DemoTrader(object):
    ...
    def execute(self, signed_user_quote):
        ...
        dst_address = self.client.eth_address(self.args.dst_address)
        self.logger.info(
...
def main():
    ...
    with NeutralApiChannel(hostport, credentials).connect() as channel:
        ...
        NUSD = filtered[0]
        trader = DemoTrader(client, config, args)
